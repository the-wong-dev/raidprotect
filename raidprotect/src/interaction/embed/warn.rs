...
pub fn not_member(user: String, lang: Lang) -> InteractionResponse {
    let embed = EmbedBuilder::new()
        .color(COLOR_RED)
        .description(lang.warn_not_member(user.remove_markdown().truncate(30)))
        .build();

    InteractionResponse::EphemeralEmbed(embed)
}

/// Bot is missing the `WARN_MEMBERS` permission
pub fn bot_missing_permission(lang: Lang) -> InteractionResponse {
    let embed = EmbedBuilder::new()
        .color(COLOR_RED)
        .title(lang.warn_bot_missing_permission_title())
...
/// Bot cannot warn due to the role hierarchy
